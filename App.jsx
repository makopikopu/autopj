...
export default function App() {
  ...
  const fetchBirdeyeDayPrice = async (chain, address, isoDate) => {
    ...
    const url = `https://public-api.birdeye.so/defi/history_price?address=${address}&address_type=token&type=1D&time_from=${dayStart}&time_to=${dayEnd}`;
  ...
  const applyContractPrice = async (chain, symbol, address) => {
    ...
    try {
      ...
      for (const isoDate of dates) {
        ...
        if (current[key] !== undefined) continue; // 既にキャッシュ済みならスキップ
        try {
          const price = await fetchBirdeyeDayPrice(chain, cleanAddress, isoDate);
      ...
      setPriceStatus(
        `${cleanSymbol}: ${dates.length}日ぶん中 ${filled}件の時価を反映しました${failed > 0 ? `(${failed}件は取得失敗)` : ""}`
      );
    } finally {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (Array.isArray(list)) {
            list.forEach((item) => {
              const mint = item.account || item.mint;
              const symbol = item.onChainMetadata?.metadata?.data?.symbol || item.legacyMetadata?.symbol;
              if (mint) solTokenSymbolCache[mint] = symbol && symbol.trim() ? symbol.trim() : null;
            });
          }
        }
      } catch (e) {
        // 失敗しても後続のフォールバック(mintの一部表示)に任せる
      }
      // 見つからなかった分もnullでキャッシュして再問い合わせを防ぐ
      uncached.forEach((m) => {
        if (solTokenSymbolCache[m] === undefined) solTokenSymbolCache[m] = null;
      });
    }
        ...
        {filteredTxs.length === 0 ? (
          ...
          <div style={styles.tableWrap}>
            <table style={styles.table}>
              <thead>
                <tr>
                  {["日時", "チェーン", "ウォレット", "種別", "銘柄", "数量", "評価額(当時)", "相手アドレス", ""].map((h) => (
